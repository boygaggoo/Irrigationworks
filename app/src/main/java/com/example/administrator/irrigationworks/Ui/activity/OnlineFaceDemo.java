...
public class OnlineFaceDemo extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		findViewById(R.id.online_regs).setOnClickListener(OnlineFaceDemo.this);
		...
		findViewById(R.id.online_detect).setOnClickListener(OnlineFaceDemo.this);
		findViewById(R.id.online_align).setOnClickListener(OnlineFaceDemo.this);
...
//            finish();
	...
	private ArrayList<String> mImagePathTasks;private void showData() {
		if (NetmImagePathDatas.isEmpty() || NetmImagePathDatas == null) {
			UtilsTool.showShortToast(OnlineFaceDemo.this, "图片不能为空");
			mProDialog.dismiss();
			return;
		}
		mProDialog = new ProgressDialog(OnlineFaceDemo.this);
		mProDialog.setCancelable(true);
		...
		for (int i = 0; i < NetmImagePathDatas.size(); i++) {
			...
			String urlpath = FileUtilcll.saveFile(OnlineFaceDemo.this, (System.currentTimeMillis() + 1) + ".jpg", map);
						...
						if (path.equals("图片上传失败") || TextUtils.isEmpty(path)) {
							...
							runOnUiThread(new Runnable() {
								...
								public void run() {
									showTip("注册成功");
									mProDialog.dismiss();
									AlertDialog.Builder builderlogin = new AlertDialog.Builder(OnlineFaceDemo.this);
									builderlogin.setMessage("请再次拍照");
									builderlogin.setTitle("");
									builderlogin.setPositiveButton("马上去拍照", new DialogInterface.OnClickListener() {
										@Override
										public void onClick(DialogInterface dialog, int which) {
											//清理数据库
//                                            Intent intentFace = new Intent(RegOnlineFaceDemo.this, RegOnlineFaceDemo.class);
...
//                mAuthid = ((EditText) findViewById(R.id.online_authid)).getText().toString());
			...
			if (schooldates != null) {
				if (schooldates.get(0).getRg().length() > 10) {
					mAuthid = "";
					mAuthid = "a" + schooldates.get(0).getRg().substring(3, 13);
